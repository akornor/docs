...
// Class to store a character history across a tree
  ...
  /**
   * Creates Character History Tree
   * @param  {Object} tree  tree object
   * @param  {Object} model  GTR-class model of nucleotide sequence evolution object 
   * @return {Object}       character tree history object
   */
...
class CharacterHistoryTree {
  ...
  /**
   * Creates sample history
   * @param  {Object} node  Node in a phylogenetic tree
   * @param  {String} startState  Initial State
   */
  ...
  sampleHistoryNode(node, startState) {
    ...
    while (sum(node.waitingTimes) < node.brl) {
      node.waitingTimes.push(
        this.model.drawWaitingTime(node.states[node.states.length - 1])
      );
      if (sum(node.waitingTimes) < node.brl) {
        node.states.push(
          this.model.drawNewState(node.states[node.states.length - 1])
        );
      }
    }
    if (node.desc != null) {
...
/**
   * Character History
   * @param  {Object} model  GTR-class model of nucleotide sequence evolution 
   * @return {Object}       character history object
   */
...
/**
  ...
  sampleHistory(brl, startState = null) {
    ...
    while (sum(this.waitingTimes) < brl) {
      this.waitingTimes.push(
        this.model.drawWaitingTime(this.states[this.states.length - 1])
      );
      if (sum(this.waitingTimes) < brl) {
        this.states.push(
          this.model.drawNewState(this.states[this.states.length - 1])
        );
      }
    }
  }
   ...
   * @param  {Object} svg  initial state 
   * @param  {Boolean} showStates  initial state 
  ...
  drawHistory(
    ...
    svg = null,
    showStates = true,
    colors = ["#ff0000", "#3cb371", "#ffa500", "#0000ff"],
    id = "charHistSVG"
  ) {
    if (svg === null) {
      svg = d3
...
/**
   ...
   * @param  {Object} bf  
   * @param  {Number} rates 
   * @param  {Array} gammaRates
   * @param  {Number} alpha
   * @param  {Boolean} invSites
   * @param  {Number} i
   * @param  {String} state 
   */
class Model {
  ...
  /**
   * buildRateMatrix
    * @return {Array} return the full matrix as a 2D array (4x4 matrix)
   */
  buildRateMatrix() {
  ...
  constructor(nodeStr, hasBrl, parent, name) {
    ...
    if (!nodeStr.includes(")")) {
      ...
      if (hasBrl && name != "root") {
        ...
        // No branch length for the root
        this.brl = null;
      }

      if (name === "root") {
        // Initiate depth values at root
        this.depth = 0;
        this.rootDist = 0;
      } else {
        this.depth = parent.depth + 1;
      ...
      for (var i = 0; i <= commaIndices.length; i++) {
        if (i === 0) {
          ...
          descStrings.push(
            nodeStr.slice(commaIndices[i - 1] + 1, commaIndices[i])
          );
        }
      }
...
class Tree {
  constructor(treeStr) {
    // Uses Newick string to instantiate a Tree object
...
function animateCircleDownCladogram(
  tr,
  w,
  h,
  scale = 0.9,
...
Tree.prototype.drawCladogram = function(
  w,
  h,
  color = "black",
  lwd = 4,
  ...
  if (node.desc != null) {
    ...
    for (var i = 0; i < node.desc.length; i++) {
      ...
      if (Math.abs(node.x - node.desc[i].x) > 0.001) {
        ...
        // Corrects for plotting artifacts with brls < lwd
        svg
          .append("line") // Horizontal lines
          .attr("x1", node.x * w * scale + (padding - lwd / 2))
          // Subtracting by lwd/2 for x1 is what keeps line intersections square
...
function drawNodeLinesWithHistory(
  node,
  svg,
  w,
  h,
  ...
  if (node.desc != null) {
    ...
    for (var i = 0; i < node.desc.length; i++) {
      svg
        ...
        .attr(
          "stroke",
          colors[findStateIndex(node.states[node.states.length - 1])]
        )
        .attr("stroke-width", lwd);
          ...
          if (
            ...
            svg
              ...
              .attr(
                ...
                node.x * w * scale +
                  waitTimeSum * w * scale +
                  node.desc[i].scaledWaitingTimes[st] * w * scale +
                  padding
              )
              .attr("y1", node.desc[i].y * h * scale + padding)
              .attr("y2", node.desc[i].y * h * scale + padding)
...
// Make member function of tree?
function drawPhylogramWithHistory(
  chHisTr,
  w,
  h,
  colors = ["#ff0000", "#3cb371", "#ffa500", "#0000ff"],
  ...
  setPhylogramXY(
    ...
    0,
    chHisTr.tr.tipNum,
    findMaxRootDist(chHisTr.tr.root)
  );
  ...
  svg
    ...
    .attr(
      "stroke",
      colors[chHisTr.model.findStateIndex(chHisTr.tr.root.states[0])]
    )
    .attr("stroke-width", lwd);
...
Tree.prototype.drawTipLabels = function(
  node,
  svg,
  w,
  h,
  ...
  if (node.desc != null) {
    ...
    svg
      ...
      .attr(
        "transform",
        "translate(" +
          (node.x * scale * w + padding + 5) +
          "," +
          (node.y * scale * h + padding + 2) +
          ")"
      )
...
function moveCirclesCladogram(nodeset, svg, currX, tipNum, moveTime) {
  ...
  if (nodeset.length < tipNum) {
    // Compares length of nodeset to number of tips
    currX = findMinX(nodeset); // Updates the current x position
    window.setTimeout(
      moveCirclesCladogram, // Name of function to call
      moveTime, // Delay time
      nodeset,
      svg,
      currX,
...
function setPhylogramXY(node, termCount, tipNum, maxDist) {
  ...
  if (node.x > 0.9999) {
    // Hacky solution to avoid rounding error for placing tips at 1
    node.x = 1;
  }
